...
If you're deploying behind a reverse proxy or load balancer, see [Deploying behind a reverse proxy](#deploying-behind-a-reverse-proxy) before turning on `cbTrustProxyHeaders` - getting this wrong either defeats rate limiting or breaks it for everyone behind the proxy.
:::
...
The `cbTrustProxyHeaders` setting (default `false`, editable at `/settings`) picks between them. Turning it on when you're not actually behind a proxy that sanitizes the header re-opens the exact rate-limit bypass it exists to close - a caller can forge a new `X-Forwarded-For` value on every login attempt and never get blocked. Leaving it off when you *are* behind such a proxy means every visitor shares the proxy's IP - one blocked caller blocks everyone behind it, and the audit trail records the proxy's address for every action.

Confirm your reverse proxy's configuration actually overwrites `X-Forwarded-For` (rather than appending to or passing through a client-supplied value) before turning this on.
